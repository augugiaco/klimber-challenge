...
{
    public abstract class GeometricShapesListPrinter
    {
        protected const string EmptySpace = " ";

        protected readonly ILanguageHelper _languageHelper;

        protected GeometricShapesListPrinter(ILanguageHelper languageHelper)
        {
        ...
        {
            if (shapes.Any() == false) return $"<h1>{_languageHelper.GetReportHeaderEmptyListText()}</h1>";
            ...
            BuildShapesDetailsAndFooter(shapes, reportContentStringBuilder);
